...
Build unsigned release artifacts locally with:
...
The app version is read from the `version` field in `package.json`. Bump that
field to change the version stamped into the build. `app.config.js` exposes the
same value to the app runtime through `EXPO_PUBLIC_BUILD_VERSION`.
