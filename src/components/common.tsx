...
export function SectionHeader({ c, title, action, onPress }: { c: Colors; title: string; action: string; onPress: () => void }) {
  return <View style={styles.sectionHeader}><Text style={[styles.sectionTitle, { color: c.text }]}>{title}</Text><Pressable onPress={onPress}><Text style={[styles.sectionAction, { color: c.primary }]}>{action}</Text></Pressable></View>;
}

export function Chip({ c, label, selected = false, onPress }: { c: Colors; label: string; selected?: boolean; onPress?: () => void }) {
  return <Pressable onPress={onPress} style={[styles.chip, { backgroundColor: selected ? c.primarySoft : c.surfaceMuted }]}><Text style={[styles.chipText, { color: selected ? c.primary : c.textMuted }]}>{label}</Text></Pressable>;
}

export function Metric({ c, label, value }: { c: Colors; label: string; value: string }) {
  return <View style={[styles.metric, { backgroundColor: c.surface, borderColor: c.border }]}><Text style={[styles.metricLabel, { color: c.textMuted }]}>{label}</Text><Text style={[styles.metricValue, { color: c.text }]}>{value}</Text></View>;
}

export function StatusPill({ c, label }: { c: Colors; label: string }) {
  return <View style={[styles.statusPill, { backgroundColor: c.primarySoft }]}><Text style={[styles.statusPillText, { color: c.primary }]}>{label}</Text></View>;
}
